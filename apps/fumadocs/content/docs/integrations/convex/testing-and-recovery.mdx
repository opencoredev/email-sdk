...
`exposeApi()` can generate app function definitions for send, batch, status, events, cancellation, and retry. It requires an authenticated Convex identity by default, records its `subject` as the email owner, and only returns or changes emails owned by that identity. Unauthenticated callers are rejected, and emails queued through server-only APIs remain private.

Use `authorize` when your app needs tenant ownership or an operation-specific policy. It receives the Convex auth context and operation name, and must return a stable owner id or `null` to deny the request. Configuration functions stay excluded unless both `includeConfigApi: true` and an explicit `authorizeConfig` admin check are provided.

### Upgrade existing public APIs

`exposeApi()` now requires Convex authentication. Configure your app's Convex auth provider before upgrading callers that use it. Existing queued emails do not have a trusted public owner, so they remain private to server-side application wrappers instead of being backfilled with guessed ownership.
