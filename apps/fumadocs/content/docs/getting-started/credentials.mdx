...
`doctor` checks whether the CLI has enough values to construct the adapter. It does not prove that a credential is accepted by the provider. Add `--live` to authenticate the key with the provider without sending an email; see the [doctor reference](/docs/reference/cli/doctor).

```bash
RESEND_API_KEY=re_xxx npx --package @opencoredev/email-sdk email-sdk doctor --adapter resend
```
