...
The SDK works in server-side Node 20+ and Bun runtimes. The installed CLI command is named `email-sdk` and requires Bun. If you only need the CLI for a quick check, run `bunx --yes @opencoredev/email-sdk ...` instead. See <a href="/docs/getting-started/install">Install</a> for the package and CLI distinction.
