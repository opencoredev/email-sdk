...
The `@opencoredev/email-sdk` package installs the `email-sdk` binary with a Node shebang. It supports Node.js 20+ and Bun 1.1+. Without a project install, run it as `npx --package @opencoredev/email-sdk email-sdk`; the unscoped `email-sdk` package on npm is unrelated.
...
| `doctor` | Check whether required values are present for one adapter. `--live` authenticates without sending; `--json` returns structured checks. |
