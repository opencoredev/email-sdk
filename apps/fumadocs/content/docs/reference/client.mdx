...
`retry` options are documented in [fallbacks and retries](/docs/concepts/fallbacks-and-retries#retries), `hooks` in [hooks](/docs/concepts/hooks), `plugins` in the [plugin API](/docs/plugins/api), and `telemetry` (plus the environment opt-outs) in [telemetry](/docs/telemetry). `providers` and `defaultProvider` are accepted [aliases](#aliases).
