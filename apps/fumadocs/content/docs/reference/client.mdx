...
`retry` options are documented in [fallbacks and retries](/docs/concepts/fallbacks-and-retries#retries), `hooks` in [hooks](/docs/concepts/hooks), and `plugins` in the [plugin API](/docs/plugins/api). Setting `telemetry: false` disables the anonymous, content-free analytics described in [telemetry](/docs/telemetry) for this client; the `EMAIL_SDK_TELEMETRY` and `DO_NOT_TRACK` environment variables disable it process-wide. `providers` and `defaultProvider` are accepted [aliases](#aliases).
