...
<TypeTable
  type={{
    ...
    telemetry: {
      description: "Set false to opt this client out of anonymous usage analytics.",
      type: "boolean",
      default: "true",
    },
  }}
...
`retry` options are documented in [fallbacks and retries](/docs/concepts/fallbacks-and-retries#retries), `hooks` in [hooks](/docs/concepts/hooks), `plugins` in the [plugin API](/docs/plugins/api), and `telemetry` in [telemetry](/docs/reference/telemetry). `providers` and `defaultProvider` are accepted [aliases](#aliases).
