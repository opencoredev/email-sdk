...
`@opencoredev/email-sdk-mcp` constructs the core client with `telemetry: false`. MCP tool outputs also exclude addresses, bodies, headers, URLs, paths, credentials, raw provider data, and causes.
