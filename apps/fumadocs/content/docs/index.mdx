---
title: Email SDK
description: Email for TypeScript apps. Send transactional email through 26 adapters with one typed SDK.
---

Email for TypeScript apps. Install the SDK, add the adapter for your existing provider account, and call `email.send()`. The message shape stays the same if you switch providers later.
...
- 25 provider API adapters plus SMTP, 26 adapters total
...
Email SDK is server-side and supports Node.js 20+ or Bun 1.1+. Use ESM imports on any supported runtime, or `require()` from CommonJS applications on Node.js `^20.19.0 || >=22.12.0` (20.19+ on 20.x, or 22.12+). On older supported runtimes, use dynamic `import()`. See [Install](/docs/getting-started/install) for typed CommonJS configuration. Keep API keys and SMTP credentials out of browser bundles.
