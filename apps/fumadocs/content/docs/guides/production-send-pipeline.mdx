...
Choose the backup from the [field support](/docs/adapters/field-support) and [capability](/docs/adapters/capability-groups) tables. SMTP supports attachments, but has no tag, metadata, or scheduling concept.
...
- Stop on `unknown` by default and reconcile provider state before retrying. `onUnknownDelivery: "stop"` only controls fallback to the next adapter; same-adapter retries follow `retry`, so set `retry: { maxAttempts: 1 }` when a durable worker owns retries.
...
The [durable PostgreSQL example](https://github.com/opencoredev/email-sdk/tree/main/packages/email-sdk/examples/durable-postgres) is a runnable version of this guide: idempotent enqueue, leased worker claims, a `needs_reconciliation` state for unknown outcomes, verified [webhook](/docs/reference/webhooks) ingestion with replay protection, and a suppression table.
