...
## 14. Confirm runtime and telemetry

v1 supports ESM imports on Node.js 20+ or Bun 1.1+, plus `require()` on Node.js `^20.19.0 || >=22.12.0` (20.19+ on 20.x, or 22.12+). On older supported runtimes, use dynamic `import()`. For typed CommonJS, use TypeScript 5.8+ with `module: nodenext`, or a compiler supporting `module: node20`; see [Install](/docs/getting-started/install). Telemetry remains enabled by default with the existing opt-outs: `telemetry: false`, `EMAIL_SDK_TELEMETRY=0`, or `DO_NOT_TRACK=1`.
