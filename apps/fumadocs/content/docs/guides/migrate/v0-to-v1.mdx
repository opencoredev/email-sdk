...
v1 supports ESM imports on Node.js 20+ or Bun 1.1+, plus `require()` on Node.js 20.19+. Telemetry remains enabled by default with the existing opt-outs: `telemetry: false`, `EMAIL_SDK_TELEMETRY=0`, or `DO_NOT_TRACK=1`.
