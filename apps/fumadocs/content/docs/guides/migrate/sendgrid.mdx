...
```bash
SENDGRID_API_KEY=SG.x npm exec --package=@opencoredev/email-sdk -- email-sdk doctor --adapter sendgrid
SENDGRID_API_KEY=SG.x npm exec --package=@opencoredev/email-sdk -- email-sdk send \
  --adapter sendgrid \
