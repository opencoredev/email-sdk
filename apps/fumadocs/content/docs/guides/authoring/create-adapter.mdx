...
- Optional: if the provider batches natively, add [`sendBulk`](/docs/reference/adapter-contract#native-batch-sends-sendbulk) so `recipientVariables` sends in one call. Skipping it is fine — the client falls back to one send per recipient.
