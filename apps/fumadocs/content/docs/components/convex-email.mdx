...
`status` covers the send lifecycle and ends at "a provider accepted the message". What happened after acceptance lives in `deliveryStatus`, filled in by [webhooks](#webhooks): `delivered`, `bounced`, or `complained`, plus `deliveredAt` on delivery. It stays unset until a matching webhook arrives.
...
Runtime config lives in a Convex table, managed with `setConfig` / `getConfig`. `setConfig` replaces the stored document rather than merging into it, so pass every field you want to keep; omitted fields are cleared:
...
Only permanent failures count as `bounced`. That is deliberate: a retryable soft bounce should not poison the status of an email that later delivers. Mailgun `failed` events map to `bounced` only when `severity` is `"permanent"`; a `"temporary"` failure lands in the event history but leaves `deliveryStatus` alone. Postmark `Bounce` records map to `bounced` only when `Type` is a permanent class (`HardBounce`, `BadEmailAddress`, `ManuallyDeactivated`); `Transient`, `SoftBounce`, and other retryable types are stored without touching the status, so a later `Delivery` still lands as `delivered`.

Bounces and complaints are sticky against `delivered`: a `delivered` event that arrives late or out of order never overwrites them. Between `bounced` and `complained` themselves, the most recent webhook wins; both mean "stop mailing this recipient", and the full sequence stays in the event history. Opens, clicks, and other events also stay in the event history without touching `deliveryStatus`.
