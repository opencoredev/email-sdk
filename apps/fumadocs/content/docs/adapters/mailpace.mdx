...
```bash
MAILPACE_API_KEY="..." npm exec --package=@opencoredev/email-sdk -- email-sdk doctor --adapter mailpace
```

```bash
MAILPACE_API_KEY="..." npm exec --package=@opencoredev/email-sdk -- email-sdk send \
  --adapter mailpace \
  --from "Acme <hello@acme.com>" \
  --to user@example.com \
