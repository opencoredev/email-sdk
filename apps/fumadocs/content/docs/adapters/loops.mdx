...
```bash
LOOPS_API_KEY="..." LOOPS_TRANSACTIONAL_ID="cm..." npm exec --package=@opencoredev/email-sdk -- email-sdk doctor --adapter loops
```

```bash
LOOPS_API_KEY="..." LOOPS_TRANSACTIONAL_ID="cm..." npm exec --package=@opencoredev/email-sdk -- email-sdk send \
  --adapter loops \
  --from "Acme <hello@acme.com>" \
  --to user@example.com \
