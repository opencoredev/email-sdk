...
```bash
LETTERMINT_API_TOKEN="lm_..." npm exec --package=@opencoredev/email-sdk -- email-sdk doctor --adapter lettermint
```

```bash
LETTERMINT_API_TOKEN="lm_..." npm exec --package=@opencoredev/email-sdk -- email-sdk send \
  --adapter lettermint \
  --from "Acme <hello@acme.com>" \
  --to user@example.com \
