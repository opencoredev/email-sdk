...
```bash
PRIMITIVE_API_KEY="prim_..." npx --package @opencoredev/email-sdk email-sdk doctor --adapter primitive
```

```bash
PRIMITIVE_API_KEY="prim_..." npx --package @opencoredev/email-sdk email-sdk send \
  --adapter primitive \
  --from "Acme <hello@acme.com>" \
  --to user@example.com \
