...
```bash
JETEMAIL_API_KEY="transactional_..." npm exec --package=@opencoredev/email-sdk -- email-sdk doctor --adapter jetemail
```

```bash
JETEMAIL_API_KEY="transactional_..." npm exec --package=@opencoredev/email-sdk -- email-sdk send \
  --adapter jetemail \
  --from "Acme <hello@acme.com>" \
  --to user@example.com \
