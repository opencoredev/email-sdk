...
```bash
SEQUENZY_API_KEY="seq_live_..." npm exec --package=@opencoredev/email-sdk -- email-sdk doctor --adapter sequenzy
```

```bash
SEQUENZY_API_KEY="seq_live_..." npm exec --package=@opencoredev/email-sdk -- email-sdk send \
  --adapter sequenzy \
  --from "Acme <hello@acme.com>" \
  --to user@example.com \
