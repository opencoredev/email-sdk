...
```bash
MAILTRAP_API_KEY="..." npm exec --package=@opencoredev/email-sdk -- email-sdk doctor --adapter mailtrap
```

```bash
MAILTRAP_API_KEY="..." npm exec --package=@opencoredev/email-sdk -- email-sdk send \
  --adapter mailtrap \
  --from "Acme <hello@acme.com>" \
  --to user@example.com \
