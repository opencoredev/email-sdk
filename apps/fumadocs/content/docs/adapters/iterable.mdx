...
<TypeTable
  type={{
    ...
    sendAt: {
      description:
        "Adapter-level schedule for the campaign target — a UTC timestamp in Iterable's expected format. Distinct from the message-level EmailMessage.sendAt field, which Iterable rejects (see the note below).",
      type: "string",
    },
    baseUrl: {
...
## Scheduling on Iterable

Iterable exposes scheduling through the adapter option above, **not** through the message field. The
two are separate contracts and easy to confuse:

- **`iterable({ sendAt })`** — set once when you configure the adapter (or via the CLI's `--send-at`
  / `ITERABLE_SEND_AT`). It is forwarded verbatim as the campaign target's `sendAt`, so it schedules
  every send this adapter makes.
- **`EmailMessage.sendAt`** — the [per-message scheduling field](/docs/reference/message#scheduled-sends)
  other adapters map to a native parameter. Iterable **rejects** it (it is not in Iterable's
  [supported fields](/docs/adapters/field-support)), so passing it throws
  `iterable does not support these EmailMessage fields: sendAt.` before any request — a fallback
  route through Iterable will fail rather than send unscheduled.

Use the adapter option for a fixed schedule; do not expect the message field to work here.
