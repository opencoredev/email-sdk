...
<TypeTable
  type={{
    ...
    dataFields: {
      ...
      type: "EmailMetadataRecord | (message) => EmailMetadataRecord",
    },
    sendAt: {
      description:
...
```bash
ITERABLE_API_KEY="..." ITERABLE_CAMPAIGN_ID="12345" npx --package @opencoredev/email-sdk email-sdk doctor --adapter iterable
```

```bash
ITERABLE_API_KEY="..." npx --package @opencoredev/email-sdk email-sdk send \
  --adapter iterable \
  --campaign-id 12345 \
  --from "Acme <hello@acme.com>" \
  --to user@example.com \
