...
```bash
ITERABLE_API_KEY="..." ITERABLE_CAMPAIGN_ID="12345" npm exec --package=@opencoredev/email-sdk -- email-sdk doctor --adapter iterable
```

```bash
ITERABLE_API_KEY="..." npm exec --package=@opencoredev/email-sdk -- email-sdk send \
  --adapter iterable \
  --campaign-id 12345 \
  --from "Acme <hello@acme.com>" \
  --to user@example.com \
