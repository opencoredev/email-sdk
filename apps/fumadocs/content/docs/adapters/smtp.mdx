---
title: SMTP
description: Send over SMTP with a standards-compliant MIME transport. No HTTP API.
icon: smtp
---
...
The SMTP adapter uses Nodemailer for the SMTP protocol, TLS, authentication, MIME composition, transfer encoding, RFC-safe subject encoding, and attachments. Point it at PurelyMail, a self-hosted server, or any provider's SMTP endpoint, and register it more than once under different names for multiple routes.
...
SMTP maps `cc`, `bcc`, `replyTo`, `headers`, and `attachments`. Nodemailer builds the MIME tree, chooses safe transfer encodings, encodes non-ASCII subjects, and supports content or path attachments (including inline `contentId` attachments). When a message has both `html` and `text`, the adapter builds a `multipart/alternative` MIME body. The generated `Message-ID` header is `<{idempotencyKey}@email-sdk.local>` when send options include an `idempotencyKey`, and a random UUID otherwise. An explicit `Message-ID` in `headers` overrides it; set one yourself if you need a specific domain for threading or deduplication.
...
<Callout type="warn" title="No tags or metadata">
  SMTP has no tag or metadata concept. Those fields throw an `EmailValidationError` before
  connecting. Attachments are supported and are encoded as MIME parts. See the
...
```bash
SMTP_HOST=smtp.purelymail.com npx --package @opencoredev/email-sdk email-sdk doctor --adapter smtp
```
