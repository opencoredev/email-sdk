...
```bash
MAILCHIMP_API_KEY="md-..." npm exec --package=@opencoredev/email-sdk -- email-sdk doctor --adapter mailchimp
```

```bash
MAILCHIMP_API_KEY="md-..." npm exec --package=@opencoredev/email-sdk -- email-sdk send \
  --adapter mailchimp \
  --from "Acme <hello@acme.com>" \
  --to user@example.com \
