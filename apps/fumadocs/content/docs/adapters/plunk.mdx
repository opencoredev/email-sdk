...
```bash
PLUNK_API_KEY="sk_..." npm exec --package=@opencoredev/email-sdk -- email-sdk doctor --adapter plunk
```

```bash
PLUNK_API_KEY="sk_..." npm exec --package=@opencoredev/email-sdk -- email-sdk send \
  --adapter plunk \
  --from "Acme <hello@acme.com>" \
  --to user@example.com \
