...
| Product/event email              | Sequenzy, Loops, Plunk                                              |
| Provider fallback for production | Resend plus SMTP, or one primary API plus Postmark                  |
| Attachment-heavy transactional   | Resend, Postmark, SendGrid, Mailgun, MailerSend, Mailtrap, Sequenzy |
...
Sequenzy metadata maps to transactional `variables`. Reserved metadata keys map to provider fields: `sequenzySlug` or `slug`, `sequenzyPreview` or `preview`, and `subscriberExternalId` or `sequenzySubscriberExternalId`.
