...
Built in, no Nodemailer. SMTP maps address fields, headers, and attachments directly into the message but has no provider-side concepts like tags or metadata.

| Adapter | CC  | BCC | Reply-To | Headers | Metadata | Tags | Attachments |
| ------- | --- | --- | -------- | ------- | -------- | ---- | ----------- |
| SMTP    | Yes | Yes | Yes      | Yes     | No       | No   | Yes         |
...
| Attachments                           | Resend, Postmark, SendGrid, Mailgun, MailerSend, SMTP |
...
await email.send({
  ...
  attachments: [{ filename: "security.txt", content: "Reset requested", contentType: "text/plain" }],
});
```
