...
Pass [`recipientVariables`](/docs/reference/message#recipient-variables) and the adapter emits one `personalizations` entry per recipient with SendGrid `substitutions`, so one API call personalizes every recipient (up to 1000, SendGrid's personalizations cap). SendGrid substitutes `%recipient.key%` tokens in `subject`, `html`, and `text` on its side:
