...
```bash
SENDGRID_API_KEY="SG...." npm exec --package=@opencoredev/email-sdk -- email-sdk doctor --adapter sendgrid
```

```bash
SENDGRID_API_KEY="SG...." npm exec --package=@opencoredev/email-sdk -- email-sdk send \
  --adapter sendgrid \
  --from "Acme <hello@acme.com>" \
  --to user@example.com \
