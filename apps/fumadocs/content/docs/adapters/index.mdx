...
| Product-triggered emails               | Sequenzy, Loops, or Plunk                                           |
| A cheap or self-managed transport      | SMTP                                                                |
| Heavy attachment support               | Resend, Postmark, SendGrid, Mailgun, MailerSend, Mailtrap, Sequenzy |

For a shorter recommendation list, use <a href="/docs/adapters/recommendations">Recommended adapters</a>.
...
- `Sponsored` means the provider has sponsored Email SDK. Sponsored adapters are still documented with the same field-support and verification language as the rest of the catalog.
...
- `Local API auth test` means the repository includes a local API-key check for that provider, but live delivery still depends on account setup.
...
| Product-led    | Sequenzy, Loops, Plunk                                                          |
