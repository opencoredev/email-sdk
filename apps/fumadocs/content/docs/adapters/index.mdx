---
title: Adapters
description: Configure one of 23 provider API adapters or the built-in SMTP transport, 24 adapters total.
---
...
| --- | --- |
| Monthly cost from 1K to 1M emails | [Pricing comparison](/docs/adapters/pricing) |
...
| Configured checks versus dated auth, send, and delivery results | [Verification evidence](/docs/adapters/verification) |
...
`doctor` checks local configuration and required settings only by default; use explicit [`doctor --live`](/docs/reference/cli/doctor) for supported provider authentication probes. `send --dry-run` checks the whole message without sending it.
