...
```bash
BREVO_API_KEY="xkeysib-..." npm exec --package=@opencoredev/email-sdk -- email-sdk doctor --adapter brevo
```

```bash
BREVO_API_KEY="xkeysib-..." npm exec --package=@opencoredev/email-sdk -- email-sdk send \
  --adapter brevo \
  --from "Acme <hello@acme.com>" \
  --to user@example.com \
