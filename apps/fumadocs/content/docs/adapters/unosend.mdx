...
```bash
UNOSEND_API_KEY="un_..." npx --package @opencoredev/email-sdk email-sdk doctor --adapter unosend
```

```bash
UNOSEND_API_KEY="un_..." npx --package @opencoredev/email-sdk email-sdk send \
  --adapter unosend \
  --from "Acme <hello@acme.com>" \
  --to user@example.com \
