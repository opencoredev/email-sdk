...
```bash
MAILERSEND_API_KEY="mlsn..." npm exec --package=@opencoredev/email-sdk -- email-sdk doctor --adapter mailersend
```

```bash
MAILERSEND_API_KEY="mlsn..." npm exec --package=@opencoredev/email-sdk -- email-sdk send \
  --adapter mailersend \
  --from "Acme <hello@acme.com>" \
  --to user@example.com \
